...
;; Design parameters for the PCB and threaded inserts used for mounting.

(define board-radius 48/2)
(define board-cutout-height 5/2)
(define board-cutout-gap 3/4)
(define board-width 16/10)
(define board-insert-thread             ; Outer thread (diameter pitch length)
  (list (+ 9/2 1/5) 1/2 13/2))          ; of the insert (here M5-0.5x12)
  ...
  (~> (fair-chassis #false)
      ...
      (apply difference
             ...
             (append
              ...
              (place-boss (scale (apply thread-cutout board-insert-thread)
                                 1 1 -1))))))
